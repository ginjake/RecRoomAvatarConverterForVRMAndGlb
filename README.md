...
1. Rec RoomからアバターをExportします。
   ...
   **TPoseまたはAPoseで出力してください。**  
   APoseで出力した場合は、変換時にポーズ選択で「A-Pose」を指定してください（CLIでは `--pose apose`）。  
...
4. GitHub Releasesから配布用zipをダウンロードします。
   ...
   [RecRoomVrmConverter.zip](https://github.com/ginjake/RecRoomAvatarConverterForVRMAndGlb/releases)
